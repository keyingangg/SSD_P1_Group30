...
# --------------------------------------------------------------------------
# Payment-record completeness (FSR-AC-10) + referential integrity (FSR-IN)
# --------------------------------------------------------------------------
def _ended_listing_with_winning_bid(seller, winner, amount="150.00"):
    """Ended listing + single winning bid, with NO Order yet (so finalize
    creates it). Distinct from _make_order, which pre-creates the Order."""
    now = timezone.now()
    listing = Listing.objects.create(
        created_by=seller,
        title="Finalize Test Item",
        description="Auction to be finalized",
        image_key="",
        category="Others",
        starting_price="100.00",
        current_highest_bid=amount,
...
@pytest.mark.django_db
def test_order_creation_on_close_populates_payment_record(verified_user):
    """Finalizing an ended auction creates an Order carrying amount/currency/listing."""
    from django.conf import settings

    seller = _other_user("seller-finalize")
    listing = _ended_listing_with_winning_bid(seller, verified_user)
...
@pytest.mark.django_db
def test_checkout_populates_ip_and_session(auth_client, verified_user):
...
@pytest.mark.django_db
def test_deleting_user_with_order_sets_winner_null(verified_user):
    """Order.winner SET_NULL preserves the payment record on a hard user delete
    (normal deletion is anonymise-in-place; this is the safety-net path)."""
    order = _make_order(verified_user)
    order_id = order.id
